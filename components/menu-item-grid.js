...
export default function ItemGrid(props) {
  ...
  return (
    <div className="flex flex-col items-center bg-white p-2 rounded-lg my-5 mx-1" id={isVegetarian ? "vege" : "non"}>
      <Image loader={bloggerLoader} src={image} alt={title} width={370} height={370} quality={40} className="rounded-[20px]" />
      <div className="text-green-600 text-3xl min-h-14 font-semibold mt-2 px-10 uppercase">{title}</div>
